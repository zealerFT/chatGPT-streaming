...
import (
	...
	chatGPT "chatGPT_streaming/grpc_stream/protos"
...
func ChatGPTStream(c *gin.Context) {
	...
	client := chatGPT.NewOpenaiServiceClient(conn)
	...
	// 处理streaming
	StreamChatCompletion := func(stream chatGPT.OpenaiService_StreamChatCompletionClient, closeStream chan struct{}) {
			...
			default:
				...
				if err != nil {
					if err == io.EOF {
						...
						// 自动上下文
						req = append(req, &chatGPT.Message{
					...
					sendWsMessage(response.GetContent())
				}
			}
		}
	}
		...
		func() {
			...
			for {
				...
				case message := <-messageWs:
					...
					stream, err := client.StreamChatCompletion(c, &chatGPT.StreamChatCompletionRequest{Token: Token, Message: req})
					if err != nil {
